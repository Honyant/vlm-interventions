...
if __name__ == '__main__':
    ...
    env = HumanInTheLoopEnv(dict(manual_control=True, use_render=True, agent_policy=TakeoverPolicy))
    success = 0
    episode_reward = []
    episode_cost = []
    ...
    while episode_num < num:
        ...
        pool.append({SampleBatch.OBS: obs.tolist(), SampleBatch.ACTIONS: action,
                     ...
                     SampleBatch.REWARDS: reward, SampleBatch.INFOS: process_info(info), SampleBatch.takeover:  takeover
                     })
        obs = new_obs
        total_reward += reward
        if done:
    ...
    for item in data_set["data"]:
        action = item[SampleBatch.ACTIONS]
        # print(action)
        # file1_data.append(item)
        # if action[1] != 0 :
        #     file2_data.append(item)
        # if action[0] != 0 and action[1] != 0:
        #     file3_data.append(item)

    file1_set = {"data": file1_data, "episode_reward": episode_reward, "episode_cost": episode_cost,
                 "success_rate": success / episode_num, "episode_len": episode_len}
    file2_set = {"data": file2_data, "episode_reward": episode_reward, "episode_cost": episode_cost,
                 "success_rate": success / episode_num, "episode_len": episode_len}
    file3_set = {"data": file3_data, "episode_reward": episode_reward, "episode_cost": episode_cost,
                 "success_rate": success / episode_num, "episode_len": episode_len}

    try:
        with open('human_traj_file1_' + str(num) + '.json', 'w') as f1:
            json.dump(file1_set, f1)
        with open('human_traj_file2_' + str(num) + '.json', 'w') as f2:
            json.dump(file2_set, f2)
        with open('human_traj_file3_' + str(num) + '.json', 'w') as f3:
            json.dump(file3_set, f3)
