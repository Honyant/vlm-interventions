...
from panda3d.core import PNMImage, Filename, StringStream
import os
import pickle
import json
import tempfile
...
class HACOCallbacks(DrivingCallbacks):
    def __init__(self, *args, **kwargs):
        ...
        self.video_dir = os.path.join("/home/anthony/HACO/haco/run_main_exp", "videos")
    ...
    def on_episode_start(
        ...
        # New: Add list for trajectory data and frames
        episode.user_data["trajectory"] = []
    ...
    def on_episode_step(
        ...
        if info is not None:
            ...
            if hasattr(env, 'engine'):
                ...
                frame = img  # Store the PNMImage object directly

            if frame is None:
                print(f"Warning: Unable to get frame at step {episode.length}")
            
            episode.user_data["frames"].append(frame)
            episode.user_data["trajectory"].append((obs, action, takeover, None))  # We'll update the frame path later
    ...
    def on_episode_end(
        ...
        for i, frame in enumerate(episode.user_data["frames"]):
            if frame is not None and i%5==0:
                frame_filename = f"frame_{i}.png"
                frame_path = os.path.join(self.video_dir, frame_filename)
                frame.write(Filename.from_os_specific(frame_path))
                episode.user_data["trajectory"][i] = (*episode.user_data["trajectory"][i][:3], frame_path)

        # Save trajectory data
        traj_data = {
            "trajectory": episode.user_data["trajectory"],
            "metrics": episode.custom_metrics
        }
        ...
        filename = f"trajectory_episode_{episode_id}.json"
        filepath = os.path.join(traj_dir, filename)

        with open(filepath, 'w') as f:
            json.dump(traj_data, f, cls=NumpyEncoder)

        print(f"Saved trajectory data for episode {episode_id} to {filepath}")
        print(f"Saved video frames for episode {episode_id} in {self.video_dir}")
