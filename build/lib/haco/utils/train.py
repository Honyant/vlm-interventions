...
# from haco.utils.callback import evaluation_config
# from haco.utils.config import baseline_eval_config

# evaluation_config = copy.deepcopy(baseline_eval_config)
# evaluation_config["use_render"] = True
# CHECKPOINT="/home/anthony/HACO/haco/run_baselines/SAC/SAC_HumanInTheLoopEnv_cff06_00001_1_seed=100_2024-05-17_03-45-28/checkpoint_1991/checkpoint-1991"
# ENV="HumanInTheLoopEnv"
# register_env("HumanInTheLoopEnv", lambda _: HumanInTheLoopEnv(evaluation_config))
def train(
        trainer,
        config,
        stop,
    ...
    # if "num_workers" in config_2:
    #     config_2["num_workers"] = min(2, config_2["num_workers"])

    # # Merge with `evaluation_config` (first try from command line, then from
    # # pkl file).
    # cc={}
    # evaluation_config = copy.deepcopy(
    # cc.get("evaluation_config", config.get(
    #         "evaluation_config", {})))
    # config_2 = merge_dicts(config_2, evaluation_config)
    # cls = get_trainable_cls("SAC")
    # agent = cls(env=ENV, config=config_2)
    ...
    # trainer.set_weights(trainer,weights)
