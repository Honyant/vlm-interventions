...
def shuffle_data(X_train, y_train, target_indices):
    # shuffle
    perm = np.arange(len(X_train))
    np.random.shuffle(perm)
    X_train = X_train[perm]
    y_train = y_train[perm]
    target_indices = np.array(target_indices)[perm]
    return X_train, y_train, target_indices

def train_model(model, X_train, y_train, path, target_indices, num_epochs=50, learning_rate=1e-3, lambda_l2=1e-5,
                batch_size=32, shuffle=True, exp_log=True, log_interval=10):
    X_train = np.array(X_train)
    y_train = np.array(y_train)
    target_indices = np.array(target_indices)
    
    # Custom loss function
    def custom_loss(predictions, targets, indices):
        batch_size = predictions.size(0)
        return torch.nn.functional.mse_loss(predictions.squeeze(), 
                                            targets[torch.arange(batch_size), indices])
    ...
    target_indices_torch = torch.from_numpy(target_indices).to(model.device).long()
    total_loss = []
    sgd_num = len(X_train_torch) / batch_size
    for t in range(num_epochs):
        if shuffle:
            X_train, y_train, target_indices = shuffle_data(X_train, y_train, target_indices)
            X_train_torch = torch.from_numpy(X_train).to(model.device).float()
            y_train_torch = torch.from_numpy(y_train).to(model.device).float()
            target_indices_torch = torch.from_numpy(target_indices).to(model.device).long()
        epoch_loss = 0
        for i in range(0, len(X_train_torch), batch_size):
            ...
            curr_indices = target_indices_torch[i:i + batch_size]
            for j in range(model.num_nets):
                preds = model.pis[j](curr_X)
                loss = criterion(preds, curr_Y, curr_indices)
