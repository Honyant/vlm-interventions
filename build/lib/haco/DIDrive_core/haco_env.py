...
class SteeringWheelController:
    ...
    def process_input(self, speed):
        pygame.event.pump()
        def apply_deadzone(value, deadzone=0.07):
            if abs(value) < deadzone:
                return 0
            return value

        def square_scaling(value):
            return value * abs(value)

        # Read joystick inputs
        raw_steering = self.joystick.get_axis(0)
        raw_throttle = self.joystick.get_axis(3)
        raw_brake = self.joystick.get_axis(7)

        # Apply deadzone elimination
        steering = apply_deadzone(-raw_steering)
        throttle = apply_deadzone((1 - raw_throttle) / 2)
        brake = apply_deadzone((1 - raw_brake) / 2)

        # Apply square scaling to steering
        steering = square_scaling(steering)
        offset = 30
